...
def benchmark_spatial_index(num_stations: int = 50000, num_queries: int = 1000):
    ...
    """
    print(f"--- Benchmarking Spatial Index ({num_stations} stations, {num_queries} queries) ---")
    
    # Generate mock data
    stations = [
        FuelStationDTO(
            id=i, name=f"Station {i}", address="", city="", state="",
    ...
    for _ in range(num_queries):
        ...
        service.find_nearby_stations(Coordinate(latitude=lat, longitude=lng), radius_miles=10.0)
