...
class ObservabilityTest(APITestCase):
    def test_correlation_id_in_response(self):
        ...
        with patch("apps.api.views.TripPlannerService.plan_optimized_trip") as mock_p:
            ...
            response = self.client.post(url, {"origin": "A", "destination": "B"}, format="json")
    ...
    def test_custom_exception_mapping(self):
        ...
        from apps.common.exceptions import ResourceNotFoundError, ServiceTimeoutError, ServiceUnavailableError
        ...
        for exc, expected_status in test_cases:
            with patch("apps.api.views.TripPlannerService.plan_optimized_trip", side_effect=exc):
                response = self.client.post(url, {"origin": "A", "destination": "B"}, format="json")
                self.assertEqual(response.status_code, expected_status, f"Failed for {type(exc)}")
