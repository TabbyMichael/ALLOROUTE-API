...
class CandidateReductionTest(SimpleTestCase):
    def setUp(self):
        ...
        self.service = CandidateReductionService(
            spatial_index=self.mock_spatial, geometry_service=self.mock_geometry
        )

    def test_reduce_candidates_logic(self):
        # Setup checkpoints every 10 miles for a 100 mile route
        checkpoints = [
            RouteCheckpoint(Coordinate(0, i / 69.0), i, 0) for i in range(0, 101, 10)
        ]
        ...
        mock_stations = [
            ...
            FuelStationDTO(
                2, "Expensive 1", "", "", "", Coordinate(0.001, 5 / 69.0), 4.00
            ),
        ...
        reduced = self.service.reduce_candidates(
            checkpoints, corridor_radius_miles=5.0, max_candidates_per_segment=1
        )
