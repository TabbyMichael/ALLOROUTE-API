...
class TestTripAPI:
    ...
    def test_optimize_trip_success(self, api_client):
        ...
        from apps.trips.domain import FuelStop, FuelStationDTO, Coordinate
        mock_result = OptimizationResult(
            route_metadata=RouteMetadata("A", "B", 1000.0, 40000, "poly"),
            fuel_stops=[
                FuelStop(
                    station=FuelStationDTO(1, "S1", "Addr", "City", "ST", Coordinate(0,0), 3.0),
                    gallons_to_buy=10.0,
                    cost=30.0,
        ...
        with patch("apps.api.views.TripPlannerService.plan_optimized_trip", return_value=mock_result):
    ...
    def test_service_failure_handling(self, api_client):
        ...
        with patch("apps.api.views.TripPlannerService.plan_optimized_trip", 
                   side_effect=ExternalServiceError("ORS is down")):
            response = api_client.post(url, payload, format="json")
            
        assert response.status_code == 400 # Our custom handler maps this to 400 by default if not mapped
