...
from apps.trips.domain import Coordinate, RouteMetadata, VehicleConfig, FuelStationDTO
...
def benchmark_optimization_logic():
    ...
    for i in range(100):
        station = FuelStationDTO(
            id=i, name=f"S{i}", address="", city="", state="", 
            coordinate=Coordinate(0,0), price_per_gallon=random.uniform(3.0, 4.5)
        )
    ...
    print(f"Optimization Time (100 nodes, 3000 miles): {statistics.mean(durations):.4f}ms")
