...
class TripOptimizationRequestSerializer(serializers.Serializer):
    ...
    origin = serializers.CharField(
        ...
        min_length=2,
        validators=[
            RegexValidator(r"^[a-zA-Z0-9\s,.-]+$", "Invalid characters in origin")
        ],
    )
    destination = serializers.CharField(
        ...
        min_length=2,
        validators=[
            RegexValidator(r"^[a-zA-Z0-9\s,.-]+$", "Invalid characters in destination")
        ],
    )
...
class FuelStationSerializer(serializers.Serializer):
    ...
    lat = serializers.FloatField(source="coordinate.latitude")
    lng = serializers.FloatField(source="coordinate.longitude")
...
class RouteMetadataSerializer(serializers.Serializer):
    ...
    def get_coordinates(self, obj):
        ...
        try:
            return polyline.decode(obj.polyline)
        except Exception:
            return []
